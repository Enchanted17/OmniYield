...
contract KittyRaffle is ERC721, Ownable {
    ...
    function _initializeCatRarities() internal {
        // 普通 - 60%
        catRarities.push(CatRarity({name: "Common Cat", probability: 6000, metadataURI: "ipfs://QmCommonCatURI"}));

        // 稀有 - 25%
        catRarities.push(CatRarity({name: "Rare Cat", probability: 2500, metadataURI: "ipfs://QmRareCatURI"}));

        // 史诗 - 10%
        catRarities.push(CatRarity({name: "Epic Cat", probability: 1000, metadataURI: "ipfs://QmEpicCatURI"}));

        // 传说 - 5%
        catRarities.push(CatRarity({name: "Legendary Cat", probability: 500, metadataURI: "ipfs://QmLegendaryCatURI"}));
    ...
    function _distributePrize(uint256 roundId, address winner, uint256 catRarityIndex) internal {
        ...
        (bool success1,) =
            treasuryVault.call{value: platformFee}(abi.encodeWithSignature("profitIn(uint256)", platformFee));
